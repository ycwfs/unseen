...
results = model.train(
  ...
  hsv_s=0.2, # 饱和度
  hsv_v=0.3, # lightness
  translate=0, # 边缘，部分物体 Translates the image horizontally and vertically by a fraction of the image size, aiding in learning to detect partially visible objects.
  ...
  # 使用红外图像时不支持马赛克，混合增强
  mosaic=0, #	目标很多，拼接出来训练不一定好 这种增强虽然好，但是由于对裁剪拼接的数据进行了训练。它会破坏检测的完整性。也就是说，如果你的检测画面中存在目标的一小部分，它也会检测出来。有时候，可能我们并不想这样。拿检测汽车来说，如果你希望只检测出完整的汽车，那么mosaic这个开关要关掉。Combines four training images into one, simulating different scene compositions and object interactions. Highly effective for complex scene understanding.
  ...
  save_period=10,
  rect=False,
  project='yolom_rgb_ir_1280_ch6_splitval_mask_fuse_hsvflip_ablu_concat',
  name='train_720_1280'
)
metrics = model.val()
