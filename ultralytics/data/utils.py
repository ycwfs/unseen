...
def verify_image_ir_label(args):
    """Verify one image-label pair."""
    ...
    try:
        ...
        ir_shape = exif_size(ir)  # image size
        ir_shape = (ir_shape[1], ir_shape[0])  # hw
        assert (ir_shape[0] > 9) & (ir_shape[1] > 9), f"ir size {ir_shape} <10 pixels"
        assert ir.format.lower() in IMG_FORMATS, f"invalid ir format {ir.format}. {FORMATS_HELP_MSG}"
